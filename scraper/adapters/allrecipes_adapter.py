...
class AllRecipesAdapter(BaseAdapter):
    ...
    def search_for_fruit(self, fruit_name: str) -> str:
        """
        Generate a search URL for finding recipes containing this fruit on AllRecipes.
    ...
    def get_recipe_urls(self, search_results_html: str, count: int = 10) -> List[str]:
        """
        Extract recipe URLs from AllRecipes search results HTML.
        
        Args:
            search_results_html (str): The HTML content of the search results page
            count (int): Number of recipe URLs to return (default: 10)
        ...
        # Extract recipe URLs using regex pattern
        # AllRecipes recipe URLs follow the pattern: /recipe/[number]/[recipe-name]/
        recipe_pattern = r'https://www\.allrecipes\.com/recipe/\d+/[^"]*'
        recipe_urls = re.findall(recipe_pattern, search_results_html)
