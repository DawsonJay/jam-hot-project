#!/usr/bin/env python3
"""
Test script to verify connection to Food.com and get HTML content.
...
from scraper.adapters.food_com_adapter import FoodComAdapter
...
def test_food_com_connection():
    """Test basic connection to Food.com and get HTML content."""
    print(f"[{get_timestamp()}] Testing Food.com connection...")
    ...
    try:
        ...
        output_file = f"food_com_search_results_{fruit_name}.html"
        ...
        if recipe_urls:
            ...
            recipe_output_file = f"food_com_recipe_{fruit_name}.html"
            with open(recipe_output_file, 'w', encoding='utf-8') as f:
                f.write(recipe_html)
            print(f"[{get_timestamp()}] ✅ Recipe HTML saved to {recipe_output_file}")
            
            print(f"[{get_timestamp()}] ✅ Food.com connection test completed successfully!")
        ...
        print(f"[{get_timestamp()}] ❌ Food.com connection test failed:")
        print(f"Error: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success = test_food_com_connection()
    if success:
